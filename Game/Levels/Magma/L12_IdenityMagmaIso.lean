...
Statement (e : α) (he: Mul.isIdentity e) (φ : α ≃* β): Mul.isIdentity (φ e) := by
  ...
  Hint (hidden := true) "Use `rw [<-hx']` to substitute `{x'}` with `{φ}({y})` in our goal."
  rw [<-hx']
...
OnlyTactic unfold rw sepcialize «let» «have»
NewTheorem MulEquiv.apply_symm_apply MulEquiv.map_mul
OnlyTheorem MulEquiv.apply_symm_apply MulEquiv.map_mul
