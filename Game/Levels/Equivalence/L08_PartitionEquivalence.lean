...
Please refer to Hr for the definition of the relation r induced by the partition C.
"
...
Statement (preamble :=
  ...
  have Hr: r = fun a b => ∃ s, s ∈ C ∧ a ∈ s ∧ b ∈ s := rfl
  ...
  · Hint "We need to show that if two elements x and y belong to the same subset in C, and y and z belong to the same subset in C, then x and z belong to the same subset in C. Use `intro x y z hxy hyz`."
    intro x y z hxy hyz
    Hint "One can unfold the definition of r using {Hr} at all places.
    Use `rw [{Hr}] at *`."
    rw [Hr] at *
